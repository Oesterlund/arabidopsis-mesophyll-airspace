cell_boundaries_dir=segmentations/cell-boundaries
cell_classes_dir=segmentations/cell-classes/00-fixed-outside
out_dir=segmentations/mesophyll-instances

mkdir -p $out_dir

instance_seg=models/instance-segmentation/boundary_instance_segmentation.py
...
for image in $images;
do
    python3 $instance_seg $cell_boundaries_dir/$image $cell_classes_dir/$image $out_dir/$image
done
