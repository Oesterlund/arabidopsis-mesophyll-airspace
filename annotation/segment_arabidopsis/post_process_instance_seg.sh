instance_seg_dir=segmentations/mesophyll-instances

pp_dir=post-processing

pp1=$pp_dir/remove_instances_touching_boundary.py
pp1_indir=$instance_seg_dir
pp1_outdir=$instance_seg_dir/01-cell-at-boundary-removed
pp1_args="--boundary-thickness 1 --boundary-is-cylinder"

pp2=$pp_dir/merge_small_instances.py
pp2_indir=$pp1_outdir
pp2_outdir=$instance_seg_dir/02-small-cells-merged
pp2_args="--small-threshold 1000"
...
for image in $images;
do
    python3 $pp1 $pp1_indir/$image $pp1_outdir/$image $pp1_args
done

# for image in $images;
# do
#     python3 $pp2 $pp2_indir/$image $pp2_outdir/$image $pp2_args
# done
