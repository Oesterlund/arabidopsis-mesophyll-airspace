semantic_seg_dir=models/almost-all/2d
semantic_seg_checkpoint=logs/04/lightning_logs/version_3/checkpoints/epoch=291-step=8469-train_loss=0.08-val_loss=0.04.ckpt
cell_boundary_seg_dir=models/mesophyll-cell-boundaries/multi-planar
cell_boundary_seg_checkpoint=logs/01/lightning_logs/version_1/checkpoints/epoch=10-step=319-train_loss=0.58-val_loss=0.57.ckpt

# Use absolute path for image_dir and relative path for out_dir
image_dir=/home/isabella/Documents/PLEN/x-ray/annotation/volumes-zoomed
out_dir=segmentations

mkdir -p segmentations/{cell-classes,cell-boundaries}
...
cd $semantic_seg_dir
python3 predict_volumes.py $semantic_seg_checkpoint $image_dir $images --outdir $cwd/$out_dir/cell-classes --batch-size 32
cd $cwd

cd $cell_boundary_seg_dir
python3 predict_volumes.py $cell_boundary_seg_checkpoint $image_dir $images --outdir $cwd/$out_dir/cell-boundaries --batch-size 32 --axes 0 1 2 --threshold 0.5
cd $cwd
